...
if last_request:
    ...
    # 将分割后的结果转换为字典形式的列表（每个元素是一个字典）
    cookie_pairs = [
        {"name": pair[0].strip(), "value": pair[1].strip()} for pair in cookie_pairs
    ]
    cookies_dict = {cookie["name"]: cookie["value"] for cookie in cookie_pairs}
    cookies_str = "; ".join([f"{name}={value}" for name, value in cookies_dict.items()])
...
#     writer = csv.writer(file)
#     writer.writerow(
#         ['昵称', '性别', '时间', '点赞', '评论', 'IP属地', '二级评论条数,条数相同说明在同一个人下面', '等级', 'uid',
#          'rpid'])
                    ...
                    if (
                        ...
                        for comment in json_data["data"]["replies"]:
                            ...
                            if vipStatus == "0":
                                ...
                                vip = comment["member"]["vip"]["label"]["text"]
                            nameplate_name = comment["member"]["nameplate"]["name"]
                            nameplate_level = comment["member"]["nameplate"]["level"]
                            is_officaial = ""
                            if (
                                str(comment["member"]["official_verify"]["type"])
                                == "-1"
                            ):
                                is_officaial = ""
                            elif (
                                str(comment["member"]["official_verify"]["type"]) == "0"
                            ):
                                is_officaial = "个人认证"
                            elif (
                                str(comment["member"]["official_verify"]["type"]) == "1"
                            ):
                                is_officaial = "机构认证"
                            officaial_desc = comment["member"]["official_verify"][
                                "desc"
                            ]
                            users.append(mid)
                            all_comments.append(
                                    ...
                                    if response.status_code == 200:
                                        ...
                                        if (
                                            ...
                                            for comment in json_data["data"]["replies"]:
                                                ...
                                                nameplate_name = comment["member"][
                                                    "nameplate"
                                                ]["name"]
                                                nameplate_level = comment["member"][
                                                    "nameplate"
                                                ]["level"]
